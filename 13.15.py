...
class AddressBook:
    def __init__(self):      
        ...
        btUpdate = Button(frame4, text = "Update",
                          command = self.update).grid(row = 1, column = 6)
        self.addressList = self.loadAddress()
        self.current = 0
        Label(frame4, text = "current address index: "+str(self.current)+"/number of addresses : " + str(len(self.addressList))).grid(row = 2, columnspan = 10, sticky = SE)
