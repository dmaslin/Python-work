...
class Graph:
    ...
    def getACycle(self):
        ...
        # Recursively search
        u = self.getACycleHelper(v, parent, searchOrders, isVisited)

        # Return a search tree
        return print(self.getPath(u, v))

    def getACycleHelper(self, v, parent, searchOrders, isVisited):
        # Store the visited vertex
        searchOrders.append(v)
        isVisited[v] = True # Vertex v visited
    ...
    def getBipartite(self):
        ...
        bipartite[0].append[0]

        for i in self.vertices:
            if i not in bipartite:
                edge = False
                for j in bipartite[0]:
                    if i in self.neighbors[bipartite[0][j]]:
                        edge = True
                if edge == False:
                    bipartite[0].append(i)
                elif edge == True:
                    edge = False
                    for j in bipartite[1]:
                        if i in self.neighbors[bipartite[1][j]]:
                            edge = True
                    if edge == False:
                        bipartite[1].append(i)
        if len(bipartite[0]) == 1 and len(bipartite[1]) == 0:
            return none
        else:
            return bipartite
