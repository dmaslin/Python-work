...
class Set:
        ...
        for i in other:
            if self.size >= self.capacity * self.loadFactorThreshold:          
                if self.capacity == MAXIMUM_CAPACITY:
                    raise RuntimeError("Exceeding maximum capacity")
                self.rehash()
            bucketIndex = hash(i) % self.capacity
            if not (i in self.table[bucketIndex]):
                self.table[bucketIndex].append(i)
                self.size += 1 # Increase size
    def difference(self, o):
        ...
        for a in range(self.size):
            t.append([])
        for i in other:
            if s >= len(t) * self.loadFactorThreshold:          
                if self.len(t) == MAXIMUM_CAPACITY:
                    raise RuntimeError("Exceeding maximum capacity")
                for a in range(len(t)):
                    t.append([])
            if not (i in self.table):
                bucketIndex = hash(i) % self.capacity
                t[bucketIndex].append(i)
                s += 1
        for i in self.table:
            if s >= len(t) * self.loadFactorThreshold:          
                if self.len(t) == MAXIMUM_CAPACITY:
                ...
                for a in range(len(t)):
                    t.append([])
            if not (i in other) and not (i in t):
                bucketIndex = hash(i) % self.capacity
                t[bucketIndex].append(i)
                s += 1
        return str(t)
    def intersection(self, other):
        ...
        for i in other:
            if s >= len(t) * self.loadFactorThreshold:          
                ...
                for a in range(len(t)):
                    t.append([])
            if (i in self.table) and not (i in t):
                bucketIndex = hash(i) % self.capacity
                t[bucketIndex].append(i)
                s += 1
        return str(t)
